...
namespace SSD1306LIB{
    ...
    void drawLine (SSD1306LIB::SSD1306 *ssd1306, uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, SSD1306LIB::WriteMode mode = SSD1306LIB::WriteMode::ADD);
    ...
    /// \param mode - mode describes setting behavior. See WriteMode doc for more information
    void drawRect (SSD1306LIB::SSD1306 *ssd1306 ,uint8_t x_start, uint8_t y_start, uint8_t x_end, uint8_t y_end, SSD1306LIB::WriteMode mode = SSD1306LIB::WriteMode::ADD);

    /// \brief Fills a rectangle from x0, y0 to x1, y1
    /// \param x_start, x_end, y_start, y_end - corner points for the rectangle
    /// \param mode - mode describes setting behavior. See WriteMode doc for more information
    void fillRect (SSD1306LIB::SSD1306 *ssd1306 ,uint8_t x_start, uint8_t y_start, uint8_t x_end, uint8_t y_end, SSD1306LIB::WriteMode mode = SSD1306LIB::WriteMode::ADD);
}
